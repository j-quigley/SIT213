...
public class Simulateur {
	...
	/**
	 * le composant Destination de la chaine de transmission analogique avec
	 * bruit
	 */
	...
	/**
	 * le composant Sonde analogique de la Destination de la chaine de
	 * transmission
	 */
	...
	public Simulateur(String[] args) throws ArgumentsException {
		...
		// ////////////////////////////////
		emetteurAnalogique = new EmetteurAnalogique(aMin, aMax, codage,
				nbEchantillons);
		transmetteurAnalogique = new TransmetteurParfaitAnalogique();
		recepteurAnalogique = new RecepteurAnalogique(aMin, aMax, codage,
				nbEchantillons);
		...
		recepteurAnalogiqueBruite = new RecepteurAnalogique(aMin, aMax, codage,
				nbEchantillons);
		...
		if (affichage == true) {
			...
			sondeSourceAnalogique = new SondeAnalogique(
					"Sonde Source Analogique");
			sondeDestinationAnalogique = new SondeAnalogique(
					"Sonde Destination Analogique");
			// Connexion//
			...
			sondeDestinationAnalogiqueBruite = new SondeAnalogique(
					"Sonde Destination Analogique avec Bruit");
			sondeDestinationBruite = new SondeLogique(
					"Sonde Destination Logique sans Bruit", 100);
			// Connexion//
			transmetteurAnalogiqueBruite
					.connecter(sondeDestinationAnalogiqueBruite);
	 ...
	 *            à transmettre (RZ par défaut)</dd>
	 *            <dt>-nbEch ne</dt>
	 *            <dd>ne (int) le nombre d'échantillons par bit (ne >= 6 pour du
	 *            RZ, ne >= 9 pour du NRZT, ne >= 18 pour du RZ, 30 par défaut))
	 *            </dd>
	 *            <dt>-ampl min max</dt>
	 *            <dd>min (float) et max (float), les amplitudes min et max du
	 *            signal analogique à transmettre ( min < max, 0.0 et 1.0 par
	 *            défaut))</dd>
	 *            <br>
	 *            <dt>-snr s</dt>
	 ...
	 *            <br>
	 *            <dt>-ti i dt ar</dt>
	 *            <dd>i (int) numero du trajet indirect (de 1 à 5), dt (int)
	 *            valeur du decalage temporel du ième trajet indirect en nombre
	 *            d'échantillons par bit, ar (float) amplitude relative au
	 *            signal initial du signal ayant effectué le ième trajet
	 *            indirect</dd>
	 *            <br>
	 *            <dt>-transducteur</dt>
	 ...
	 *            <br>
	 *            <dt>-aveugle</dt>
	 *            <dd>les récepteurs ne connaissent ni l'amplitude min et max du
	 *            signal, ni les différents trajets indirects (s'il y en a).</dd>
	 *            <br>
	 *            </dl>
	 * <br>
	 *            <b>Contraintes</b> : Il y a des interdépendances sur les
	...
	public void analyseArguments(String[] args) throws ArgumentsException {
		...
		for (int i = 0; i < args.length; i++) {
			...
			if (args[i].matches("-s")) {
				...
				i++;
				if ((args[i].matches("RZ")) | (args[i].matches("NRZ"))
						| (args[i].matches("NRZT"))) {
					codage = args[i];
				} else {
					throw new ArgumentsException(
							"Valeur de parametre -form invalide : " + args[i]);
				}
				...
				if (args[i].matches("[1-9][0-9]*")) {
					...
					throw new ArgumentsException(
							"Valeur de parametre -nbEch invalide : " + args[i]);
				}
				...
				if (args[i].matches("-?[0-9]+(\\.[0-9]+)?")) {
					...
					if ((args[i].matches("-?[0-9]+(\\.[0-9]+)?"))
						...
						throw new ArgumentsException(
								"Valeur de parametre -ampl invalide : "
										+ args[i]);
					}
				} else {
					throw new ArgumentsException(
							"Valeur de parametre -ampl invalide : " + args[i]);
				}
				...
				if (args[i].matches("-?[1-9][0-9]*")) {
					...
					throw new ArgumentsException(
							"Valeur de parametre -snr invalide : " + args[i]);
				}
			} else
	...
	public void execute() throws Exception {
		try {
			...
			throw new Exception(
					"Erreur lors de l'envoi sur la chaine de transmission");
		}
	}
	...
	public float calculTauxErreurBinaire() {
		...
		Information<Boolean> informationRecue = destinationAnalogiqueBruite
				.getInformationRecue();

		float nbErreur = 0;

		for (int i = 0; i < informationEmise.nbElements(); i++) {
			if (informationEmise.iemeElement(i) != informationRecue
					.iemeElement(i)) {
				nbErreur++;
			}
		}
